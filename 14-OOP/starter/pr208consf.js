...
// Challenge, I want constructor function where I can give name, age, country, gender as argument
const checkPerson = function (name, birthYear, country, gender) {
  this.name = name;
  this.birthYear = birthYear;
...
const ansa = new checkPerson('Ansa Khalid', 2002, 'Pakistan', 'Female');
console.log(ansa);
console.log(ansa instanceof checkPerson);
console.log(ansa.calcAge());
const priya = new checkPerson('Priya', 1998, 'India', 'Female');
console.log(priya);
console.log(priya instanceof checkPerson);
...
const book = function (title, author, pages) {
  ...
  this.read = function () {
    console.log(
      `You've read ${
        Math.floor(Math.random() * this.pages) + 1
      } pages of total ${this.pages} pages of ${this.title} written by ${
        this.author
      }`
    );
  };
};
const gameOfThrones = new book('Game of Thrones', 'GG Martin', 200);
console.log(gameOfThrones);
console.log(gameOfThrones.read());
