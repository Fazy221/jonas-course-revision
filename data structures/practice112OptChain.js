...
for (const day of days) {
  ...
  console.log(`Restaurant on ${day} opens at ${open}.`);
}
// Methods (can check if function method exist before calling)
console.log(restaurant.order?.(1, 2) ?? 'not exist');
console.log(restaurant.orderRosito?.(1, 2) ?? 'no function exist');
...
console.log(user[0]?.name ?? 'no array exist');
console.log(user[3]?.name ?? 'no array exist');
